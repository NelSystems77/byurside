...
**Last Updated:** May 13, 2026 (v2.12)
...
| Respuestas largas de Danay se clipeaban sin scroll | `VisualBridge.tsx` | Añadido `max-h-[32vh] sm:max-h-[38vh] overflow-y-auto overscroll-contain` al contenedor del texto; fuentes responsive (`text-lg sm:text-2xl lg:text-3xl` para 'grande'); padding y border-radius responsive. |
| Botón micrófono fijo `w-40 h-40` desbordaba en iPhone SE | `Dashboard.tsx` | Cambiado a `w-32 h-32 sm:w-40 sm:h-40`; border `border-4 sm:border-8`; íconos internos con className responsive (`w-9 h-9 sm:w-12 sm:h-12`). |
| `main` con `overflow-hidden justify-center` cortaba contenido en pantallas pequeñas | `Dashboard.tsx` | Refactorizado a `overflow-y-auto min-h-0`; contenido en div interno con `justify-center my-auto` para centrar cuando hay espacio y hacer scroll cuando no. |
| Paddings fijos bloqueaban layout en móviles | `Dashboard.tsx`, `RoleSelector.tsx` | Header: `pt-6 sm:pt-10 px-4 sm:px-6`; footer: `pb-8 sm:pb-12 px-4 sm:px-6`; input: `py-3 sm:py-4 text-base sm:text-xl`; modal SOS: `p-5 sm:p-8 rounded-[28px] sm:rounded-[40px]`; botones RoleSelector: `p-4 sm:p-6 rounded-[20px] sm:rounded-[28px]`. |
...
| Bucle infinito de reintentos tras la primera escucha exitosa | `useEscuchar.ts` | El efecto de auto-retry dependía de `[autoRetry, _iniciarRetry]`. `_iniciarRetry` se recrea en cada cambio de `escuchando`; como `autoRetry` nunca se resetea a 0 tras retries agotados, cada flip de `escuchando` re-disparaba el efecto → nuevo timeout → nuevo retry → nuevo flip → bucle. Fix: `_iniciarRetryRef` estabiliza el callback con un ref; el efecto pasa a depender solo de `[autoRetry]`. |
...
**Document Version:** 2.13 | **Updated:** May 14, 2026  
**Cambios v2.13:** Bug de bucle infinito en `useEscuchar.ts` corregido — tras la primera escucha exitosa, un error `network` elevaba `autoRetry` a 1 y nunca volvía a 0; cada flip de `escuchando` recreaba `_iniciarRetry`, re-disparaba el efecto (que dependía de `[autoRetry, _iniciarRetry]`), programaba otro retry → loop. Fix: patrón "stable callback via ref" con `_iniciarRetryRef`; efecto pasa a depender solo de `[autoRetry]`.  
**Cambios v2.12:** Responsividad completa en tres componentes: (1) `VisualBridge` — mensajes de Danay ahora tienen scroll interno (`max-h-[32vh] sm:max-h-[38vh] overflow-y-auto`), fuentes responsive (`text-lg sm:text-2xl lg:text-3xl` para modo 'grande'), padding y bordes responsive. (2) `Dashboard` — `main` refactorizado de `overflow-hidden justify-center` a `overflow-y-auto min-h-0` con contenedor interno centrado via `my-auto`; botón micrófono `w-32 h-32 sm:w-40 sm:h-40` con border e íconos responsive; header, agua panel, footer e input con paddings `sm:` adaptativos; modal SOS con bordes y paddings responsive. (3) `RoleSelector` — título `text-3xl sm:text-4xl`, botones `p-4 sm:p-6 rounded-[20px] sm:rounded-[28px]`, íconos `w-12 sm:w-14`, textos descriptivos `text-xs sm:text-sm`, spacing `mb-8 sm:mb-12 mt-6 sm:mt-10`.  
